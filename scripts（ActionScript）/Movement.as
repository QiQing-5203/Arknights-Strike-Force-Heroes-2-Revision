package
{
   public class Movement
   {
            ...
            if(!this.crouching)
            {
               ...
               if(!this.landHard)
               {
                  ...
                  else
                  {
                     ...
                     this.unit.MC.goto("climb" + (this.climbSize != 1 ? "big" : "small"));
                  }
                  return;
               }
               return;
      ...
      public function EnterFrame() : void
      {
         ...
         var _loc6_:uint = 0;
         if(!this.unit.game.gameStarted || Boolean(this.unit.status.sSpawn))
         {
            this.unit.keys = 0;
         }
         if(!(!this.jumping && this.unit.keys & this.unit.DOWN && !this.landHard))
         {
            if(!(this.crouching && (Boolean(this.hitTest(-17,-45)) || Boolean(this.hitTest(17,-45)))))
            {
               this.crouching = false;
            }
            else
         ...
         {
            if(!(this.unit.keys & this.unit.RIGHT && !this.landHard && !this.unit.status.sSpawn))
            {
               this.unit.nextAnim = "idle";
               _loc4_ = (!this.jumping ? (!this.crouching ? this.xBrake : this.xCrouchBrake) : this.xAirBrake) * this.modBrake;
               if(this.xVel > _loc4_)
               {
                  this.xVel -= _loc4_;
               }
            ...
            else
            {
               ...
               if(!this.crouching)
               {
                  ...
                  this.xVel += (!this.jumping ? this.xAcc : this.xAirAcc) * this.modSpeed;
         ...
         if(!(Boolean(this.climb) && this.jumpClimb))
         {
            if(!this.climb)
            {
               while(true)
               {
                  if(Boolean(this.hitTest(0,8)) && !this.hitTest(0,-1))
                  {
                     this.unit.y += 0.5;
                     continue;
                  }
            ...
            else
            {
               while(true)
               {
                  if(Boolean(this.hitTest(0,6)) && !this.hitTest(0,-1))
                  {
                     this.unit.y += 0.5;
                     continue;
                  }
         ...
         {
            while(Boolean(this.hitTest(this.climb * 5,14)) && !this.hitTest(this.climb * 5,-1))
            {
               this.unit.y += 0.5;
            }
         }
         ...
         else
         {
            ...
            if(this.yVel > 0)
            {
               ...
               {
                  if(!(this.unit.keys & this.unit.RIGHT))
                  {
         ...
         {
            if(this.unit.keys & this.unit.RIGHT && this.hitTest(17,-20) && !this.hitTest(17,-55))
            {
               this.climbSize = 1;
               _loc2_ = true;
            }
         ...
         if(!this.crouching)
         {
            while(true)
            {
               if(Boolean(this.hitTest(17,-20)) || Boolean(this.hitTest(17,-25)) || Boolean(this.hitTest(17,-35)) || Boolean(this.hitTest(17,-45)))
               {
                  --this.unit.x;
                  continue;
               }
         ...
         {
            if(this.unit.keys & this.unit.LEFT && this.hitTest(-17,-20) && !this.hitTest(-17,-55))
            {
               this.climbSize = 1;
               _loc3_ = true;
            }
